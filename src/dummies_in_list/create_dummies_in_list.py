...
def write_dummies(df, l, out_fp, index_label):
    new_cols = []
    s1, s2 = 's1', 's2'
    df['s1'] = df[tokens_q1].apply(lambda s: set(s.split()))
    df['s2'] = df[tokens_q2].apply(lambda s: set(s.split()))

    for name in l:
        in_q1 = '{}_in_q1'.format(name)
        in_q2 = '{}_in_q2'.format(name)
        in_both = '{}_in_both'.format(name)

        df[in_q1] = df[s1].apply(lambda s: name in s)
        df[in_q2] = df[s2].apply(lambda s: name in s)
        df[in_both] = df[in_q1]&df[in_q2]

        cols = [in_q1, in_q2, in_both]
        for col in cols:
            df[col] = df[col].apply(lambda s: 1 if s else 0)

        new_cols+=cols

    # return new_cols
    df[new_cols].to_csv(out_fp, index_label=index_label)
...
def write_top_25_uppers_dummies():
    uppers = load_top_110_uppers()[:25]
