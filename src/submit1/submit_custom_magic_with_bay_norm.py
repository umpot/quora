...
def apply_bayesian_normalization(df, dups_num_col, all_num_col,new_col, k=5, f=1, g=1, r_k=0.01):
    ...
    df['freq'] = df[dups_num_col]/df[all_num_col]
    df[new_col] = df["lambda"] * df["freq"] + (1 - df["lambda"]) * prior_prob
    df[new_col] = df[new_col] * np.random.uniform(1 - r_k, 1 + r_k, len(df))
...
def custom_magic_with_update(bf_train, bf_test, update_df):
    ...
    apply_bayesian_normalization(bf_train, q1_as_q1_dups_num, q1_as_q1_count, q1_as_q1_dup_freq)
    # bf_train[q2_as_q2_dup_freq] = (bf_train[q2_as_q2_dups_num]) / (bf_train[q2_as_q2_count])
    apply_bayesian_normalization(bf_train, q2_as_q2_dups_num, q2_as_q2_count, q2_as_q2_dup_freq)
    ...
    apply_bayesian_normalization(bf_test, q1_as_q1_dups_num, q1_as_q1_count, q1_as_q1_dup_freq)
    # bf_test[q2_as_q2_dup_freq] = (bf_test[q2_as_q2_dups_num]) / (bf_test[q2_as_q2_count])
    apply_bayesian_normalization(bf_test, q2_as_q2_dups_num, q2_as_q2_count,q2_as_q2_dup_freq)
...
name='submit_custom_magic_with_bay_norm'
submit_xgb(name)
