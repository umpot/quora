...
def prepare_df(df):
    for col in [postag_q1, postag_q2]:
        df[col] = df[col].apply(literal_eval)

def get_nnps(p):
    return [x[1] for x in p if x[2] in NNP_TAGS]

def get_nns(p):
    return [x[1] for x in p if x[2] in NNS_TAGS]
...
def add_nn_cols(df):
    prepare_df(df)
    df[nnp_q1]=df[postag_q1].apply(get_nnps)
    df[nnp_q2]=df[postag_q2].apply(get_nnps)

    df[nn_q1]=df[postag_q1].apply(get_nns)
    df[nn_q2]=df[postag_q2].apply(get_nns)
