...
def del_trash_cols(df):
    for col in [qid1, qid2, question1, question2]:
        if col in df:
            del df[col]

def test_fm():
    clf = FMClassification()
    ...
    df[TARGET] = df[TARGET].apply(lambda s: -1 if s==0 else 1)


    folds = get_dummy_folds(df)
    ...
    proba = clf.fit_predict_proba(train.values, train_target, test.values)
