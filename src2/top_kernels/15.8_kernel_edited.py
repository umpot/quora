...
def main():
    ...
    X_valid, y_valid = down_sample(X_valid, y_valid)
...
def down_sample(X_train, y_train):
    ...
    sz1 = int(0.8 * len(pos_train))
    X_train = pd.concat((neg_train, pos_train.iloc[:sz1], neg_train))
    y_train = np.array([0] * neg_train.shape[0] + [1] * pos_train.iloc[:sz1].shape[0] + [0] * neg_train.shape[0])
