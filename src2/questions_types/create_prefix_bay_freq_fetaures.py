...
def process_train_test_bi_pref(train_df, test_df, update_df):
    ...
    update_df.loc[test_df.index, col] = bl.loc[test_df.index, col]
...
def get_bi_prefix_counter(df):
    return Counter(list(df[bi_prefix_q1])+list(df[bi_prefix_q2]))


bi_prefix_dummies_50_train_fp=os.path.join(data_folder,'prefix','bi_prefix_dummies_50_train_fp.csv')
bi_prefix_dummies_50_test_fp=os.path.join(data_folder,'prefix','bi_prefix_dummies_50_test_fp.csv')

def write_top_N_bi_prefix_dummies(N):
    train_df, test_df = load_train(), load_test()

    for df in [train_df, test_df]:
        add_bi_prefix_cols(df)

    c=get_bi_prefix_counter(train_df)
    topN = {x[0] for x in c.most_common(N)}
    print topN


    for df in [train_df, test_df]:
        df[bi_prefix_q1] = df[bi_prefix_q1].apply(lambda s: s if s in topN else None)
        df[bi_prefix_q2] = df[bi_prefix_q2].apply(lambda s: s if s in topN else None)

    train_df = pd.get_dummies(train_df, columns=[bi_prefix_q1, bi_prefix_q2])
    test_df = pd.get_dummies(test_df, columns=[bi_prefix_q1, bi_prefix_q2])

    new_cols = ['{}_{}'.format(bi_prefix_q1, x) for x in topN]+['{}_{}'.format(bi_prefix_q2, x) for x in topN]

    train_df[new_cols].to_csv(bi_prefix_dummies_50_train_fp, index_label='id')
    test_df[new_cols].to_csv(bi_prefix_dummies_50_test_fp, index_label='test_id')


def load_bi_pref_dummies_50_train():
    return pd.read_csv(bi_prefix_dummies_50_train_fp, index_col='id')

def load_bi_pref_dummies_50_test():
